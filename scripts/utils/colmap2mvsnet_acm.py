...
from .get_score_auto import get_score
...
def read_model(path, ext):
    if os.path.exists(os.path.join(path, "cameras.txt")):
    ...
    elif os.path.exists(os.path.join(path, "cameras.bin" )):
...
def my_calc_score(inputs, images, points3d, extrinsic):
    i, j = inputs
    id_i = images[i + 1].point3D_ids
    id_j = images[j + 1].point3D_ids
    id_intersect = [it for it in id_i if it in id_j]
...
def processing_single_scene(args):
    ...
    for i in range(num_images):
        ...
        # relaxed depth range
        if len(zs_sorted) == 0:
            print("Error: The zs_sorted list is empty. Please check the data.")
            depth_min = 0.1
            depth_max = 999
        elif int(len(zs) * .01) >= len(zs_sorted) or int(len(zs) * .99) >= len(zs_sorted):
            print("Error: Index out of range. Please check the data.")
        else:
        ...
        with open(os.path.join(cam_dir, '%08d_cam.txt' % i), 'w') as f:
            ...
            for j in range(3):
                for k in range(3):
                    ...
                    if args.resize == 1:
                        f.write(str(intrinsic[images[i + 1].camera_id][j, k] / 2) + ' ')
    ...
    for i in range(num_images):
        ...
        if args.resize == 1:
            ...
            cv2.imwrite(os.path.join(image_converted_dir, '%08d.jpg' % i), resized_img)
        else:
            shutil.copyfile(os.path.join(image_dir, images[i + 1].name),
                            os.path.join(image_converted_dir, '%08d.jpg' % i))

def process_image(image_path, output_path, index):
    img = cv2.imread(image_path)
    resized_img = cv2.resize(img, (960, 540))
    #resized_img = cv2.resize(img, (480, 270))
    cv2.imwrite(output_path % index, resized_img)

def my_processing_single_scene(dense_folder,save_folder):
    image_dir = os.path.join(dense_folder, 'images')
    model_dir = os.path.join(dense_folder, 'sparse')
    cam_dir = os.path.join(save_folder, 'cams')
    image_converted_dir = os.path.join(save_folder, 'images')
    sfm_path = os.path.dirname(dense_folder)
    ...
    '''p = mp.Pool(processes=mp.cpu_count())
    func = partial(my_calc_score, images=images, points3d=points3d,  extrinsic=extrinsic)
    result = p.map(func, queue)
    for i, j, s in result:
        score[i, j] = s
        score[j, i] = s'''
    # score = get_pair_txt(num_images, score)
    score = get_score(sfm_path, score)
    view_sel = []
    num_view = min(20, len(images) - 1)
    for i in range(len(images)):
        sorted_score = np.argsort(score[i])[::-1]
            ...
            for j in range(3):
                for k in range(3):
                    if j == 2 and k == 2:
                        ...
                        continue
                    f.write(str(intrinsic[images[i + 1].camera_id][j, k] / 2) + ' ')
                    # f.write(str(intrinsic[images[i + 1].camera_id][j, k] / 4) + ' ')
    ...
    for i in range(num_images):
        img_path = os.path.join(image_dir, images[i + 1].name)
        output_path = os.path.join(image_converted_dir, '%08d.jpg')

        # 异步执行process_image函数
        pool.apply_async(process_image, args=(img_path, output_path, i))
